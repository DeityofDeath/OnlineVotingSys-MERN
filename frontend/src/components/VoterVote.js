...
function VoterVote() {
    ...
    const fetchCandidates = () => {
        axios.get(`/voters-api/candidates/${electionTitle}`)
            .then(response => {
    ...
    async function handleVoteClick(candidateId) {
        ...
        if (confirmVote) {
          let response= await  axios.post(`/voters-api/vote/${electionTitle}/${voterName}/${candidateId}`)
                        ...
                        <div key={candidate.id} className="col">
                            <div className="card h-100">
                                ...
                                <div className="card-footer text-center">
                                    <button onClick={() => handleVoteClick(candidate.id)} className="btn btn-primary">Vote</button>
                                </div>
                            </div>
                        </div>
                    ))}
